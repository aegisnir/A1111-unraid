...
monitor_webui_output() {
  ...
  while IFS= read -r line; do
    ...
    if [[ $_saw_checkpoint -eq 0 && "${line}" == *"No checkpoints found"* ]]; then
      _saw_checkpoint=1
      echo ""
      echo "  ${C_WARN}┌─ [KNOWN WARNING] ───────────────────────────────────────────────────┐"
      echo "  │  No model checkpoint was found. This is expected on a fresh install │"
      echo "  │  or if /data/models/ was cleared. The WebUI will still start.       │"
      echo "  │                                                                     │"
      echo "  │  Fix: add a .safetensors or .ckpt file to:                          │"
      echo "  │         /data/models/Stable-diffusion/                              │"
      echo "  │  then restart the container, or use Settings → Refresh in the UI.   │"
      echo "  └─────────────────────────────────────────────────────────────────────┘${C_RESET}"
      echo ""
    fi
    ...
    if [[ "${line}" == *"xFormers was built for:"* && "${line}" == *"cu128"* ]]; then
      echo ""
      echo "  ${C_CRIT}┌─ [XFORMERS MISMATCH] ───────────────────────────────────────────────┐"
      echo "  │  xformers was installed from PyPI (cu128 build) but torch is cu130.  │"
      echo "  │  CUDA extensions will not load and performance will be reduced.      │"
      echo "  │                                                                     │"
      echo "  │  Fix: delete the stale venv so the container rebuilds it cleanly:   │"
      echo "  │    rm -rf /mnt/user/ai/data/venv                                   │"
      echo "  │  (adjust path to match your data share), then restart container.    │"
      echo "  └─────────────────────────────────────────────────────────────────────┘${C_RESET}"
      echo ""
    fi
    ...
    if [[ "${line}" == *"CUDA out of memory"* || "${line}" == *"OutOfMemoryError"* ]]; then
      echo ""
      echo "  ${C_CRIT}┌─ [GPU MEMORY ERROR] ────────────────────────────────────────────────┐"
      echo "  │  Your GPU ran out of VRAM during this operation.                    │"
      echo "  │                                                                     │"
      echo "  │  Quick fix (add as container env var, then restart):                │"
      echo "  │    PYTORCH_ALLOC_CONF = expandable_segments:True                   │"
      echo "  │    Reduces fragmentation; often resolves marginal OOM.              │"
      echo "  │                                                                     │"
      echo "  │  Other tips:                                                        │"
    ...
    if [[ "${line}" == *"torch.cuda.is_available() = False"* || "${line}" == *"Torch is not able to use GPU"* ]]; then
      ...
      echo "  │  PyTorch cannot see a CUDA-capable GPU.                             │"
      echo "  │  Check:                                                             │"
      echo "  │    • --runtime=nvidia is present in Extra Parameters in the template│"
      echo "  │    • The Unraid NVIDIA plugin is installed and working              │"
      echo "  │    • Run on the host to verify:                                     │"
      echo "  │        docker run --rm --runtime=nvidia ubuntu nvidia-smi           │"
      echo "  └─────────────────────────────────────────────────────────────────────┘${C_RESET}"
      echo ""
    fi
